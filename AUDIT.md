...
- Fixed a reported false “Speech unavailable” state: Android 11+ package visibility now declares `android.speech.RecognitionService`, with an on-device recognizer fallback when available. The final APK was installed on the connected phone; a gesture invocation selected the Google recognizer, opened its microphone and rendered SOL's `Listening…` state. A spoken transcript after this final installation remains a short human check.
- Android speech error code 11 (`ERROR_SERVER_DISCONNECTED`) now clears the stale speech engine and leaves SOL open with a Talk-again/type fallback. The current crash buffer contained no SOL Java exception; exit history showed recent package updates/force-stops and one older low-memory kill, so a repeated crash needs a fresh device trace before making broader lifecycle changes.
