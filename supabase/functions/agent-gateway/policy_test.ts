...
Deno.test('server controls model cost and disables parallel tools',()=>{
 const body=validateRequest({...sample(),model:'expensive-model',max_output_tokens:999999});
 assert(body.model==='gpt-6-astra' && body.max_output_tokens===4096 && body.parallel_tool_calls===false);
});
Deno.test('rejects hosted and unregistered tools',()=>{
 for(const tools of [[{type:'web_search'}],[{type:'function',name:'execute_shell',parameters:{}}]]) {
  let rejected=false;try{validateRequest({...sample(),tools});}catch{rejected=true;}assert(rejected);
 }
});
Deno.test('rejects extra request fields and invalid continuation',()=>{
 for(const extra of [{background:true},{previous_response_id:'https://example.com'},{input:null}]) {
  let rejected=false;try{validateRequest({...sample(),...extra});}catch{rejected=true;}assert(rejected);
 }
});
